...
class AircraftExperiment:
    ...
    def __init__(self, model, run_name, experiment_id, projection_title=None, 
                 visualize_multiclass=False, data_source='local', config_path='config.yaml'):
        """
        ...
        self.visualize_multiclass = visualize_multiclass
    ...
    def run(self):
        ...
        with mlflow.start_run(run_name=self.run_name, experiment_id=self.experiment_id):
            ...
            self.results = self.evaluate(images=self.images, labels=self.binary_labels)
            ...
            if self.config['make_plots']:
                ...
                self.figure_path = f'results/figs/{figure_name}'
                
                plt.ioff()
                
                # choose binary or multiclass labels
                if self.visualize_multiclass:
                    viz_labels = self.labels
                else:
                    viz_labels = self.binary_labels
                self.projection = visualizations.plot_feature_vectors(embeddings, 
                                                    viz_labels,
                                                    title=self.projection_title,
                                                    method='isomap',
                                                    file_path='results/figs/',
